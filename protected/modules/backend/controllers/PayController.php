	...
	*/
	...
	public function actionAjaxUpdateNotCashInfo($pay_not_cash_id)
	{
		...
		if(isset($_POST['PayNotCashInfo']))
		{
			...
			if($model->save())
			{
				echo CHtml::tag("script",array(), 'jQuery("#updatePayNotCashInfoDialog").modal("hide");$("#btnRefreshNotCashInfo").click();');
				Yii::app()->end();
			}
		}
		...
		$cs->scriptMap['jquery.formatCurrency-1.4.0.js'] = false;
		$cs->scriptMap['jquery.formatCurrency.es-VE.js'] = false;
                $cs->scriptMap['bootstrap-datepicker.min.js'] = false;
                $cs->scriptMap['bootstrap-datepicker.es.js'] = false;
                //fc749be/js/jquery-ui-no-conflict.min.js
